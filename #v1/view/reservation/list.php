					  ...
					  <table id="dataList" class="table table-bordered table-striped">
						<thead>
							<tr style="background-color: #A7DCFF;color: white">
								...
								<th width="15%">Tarikh / Masa Dipinjam</th>
								<th width="15%">Tarikh / Masa Pemulangan</th>
            ...
            $.ajax({
              ...
              success: function(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                 ...
                 var detailPeminjam = '<a href="#" onclick="detailPeminjam('+data[i]['user_id']+')"> '+data[i]['user_name']+'  </a>';

                 var maklumatBtn = '<center> <a href="reservation-view/'+data[i]['reservation_id']+'" class="btn btn-sm btn-primary"> Maklumat  </a> </center>';

                 var convertTime = timeFormatter(data[i]['reservation_time_pickup']);

                 console.log(data[i]['reservation_time_pickup']);

                  table.row.add([
                    detailPeminjam,
                    data[i]['user_staff_id'],
                    dateFormater(data[i]['reservation_date_pickup']) + " / " + timeFormatter(data[i]['reservation_time_pickup']),
                    dateFormater(data[i]['reservation_date_return']) + " / " + timeFormatter(data[i]['reservation_time_return']),
                    maklumatBtn,
                  ]).draw();
